...
//Arthur Franco Neto
void PICC_BlockMifareClassicSectorToSerial(
	MFRC522Ptr_t mfrc,
	...
	for (int8_t blockOffset = no_of_blocks - 1; blockOffset >= 0;
		...
		printf("\r\n");
		uint8_t index =0;
		uint8_t uint8_tsToCopy = 10;
		uint8_t uidIndex = blockAddr * 10 - 10;
		for (int count = 0; count < uint8_tsToCopy; count++) {
			uid->eeprom[uidIndex + count] = buffer[index++];
		}
	}
	return;
}

void PICC_BlockMifareClassicToSerial(	MFRC522Ptr_t mfrc,
	Uid *uid,			///< Pointer to Uid struct returned from a successful
						///PICC_Select().
	...
	for (int8_t i = no_of_sectors - 1; i >= 0; i--) {
		PICC_BlockMifareClassicSectorToSerial(mfrc, uid, key, 0, block);
	}
...
void PICC_ReadBlock(MFRC522Ptr_t mfrc, Uid *uid, uint8_t block){
	MIFARE_Key key;
	...
	PICC_BlockMifareClassicToSerial(mfrc, uid, piccType, &key, block);

	printf("\r\n");
	PICC_HaltA(mfrc); // Already done if it was a MIFARE Classic PICC.
}

// fim Arthur Franco Neto
